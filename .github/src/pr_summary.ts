...
export const updateSummary = async (
  { github, context, core }: ActionArgs,
  prNumber: number,
...
export const initSummary = async (
  ...
  const body = `${originalSummary}${botDelimiter}${
    prTemplate.split(botDelimiter)[1]
  }`;
...
export const hasCheckboxChanged = async (
  ...
  const hasCheckboxUpdated =
    event.changes.body.from.includes(prevState) &&
    event.pull_request.body.includes(nextState);
  log(
    hasCheckboxUpdated ? "cyan" : "reset",
    `hasCheckboxUpdated ${checkBoxText} changed: ${hasCheckboxUpdated}`
  );
...
export const checkSummary = createPrAction(async (args) => {
  ...
  if (
    event.action !== "edited" ||
    !event.pull_request.body.includes(botDelimiter)
  ) {
    await initSummary(
      args,
        ...
        (body: string) => {
          ...
          if (/Vercel/.test(checkBoxText)) {
            newBody = newBody.replace(
              `- [x] ${checkBoxText}`,
              `- ${checkBoxText} deployment triggered ☑️`
            );
          } else {
            newBody = newBody.replace(
              `- [x] ${checkBoxText}`,
              `- ${checkBoxText} Running ⏳`
            );
          }
  ...
  if (changedValue) {
    ...
    if (/Vercel/.test(changedValue ?? "")) {
      await createVercelDeploymentStg({
        ...args,
        process: {
          ...args.process,
          env: { ...args.process.env, CHECK: changedValue },
        },
      });
    }
    return changedValue;
