...
export const runCommand = (command: string, args: string[], env: Record<string, string>) => {
    log(
        'magenta',
        `Running command ${command} with args ${args.join(', ')} and env ${Object.keys(env)
            .filter((key) => key.includes('CHROMATIC_'))
            .map((key) => `${key}=${env[key]}`)
            .join(', ')}`
    );
    const child = childProcess.spawnSync(command, args, { encoding: 'utf8', env });
    return child;
};

const runChromaticCommand = ({ processEnvs, ...chromaticArgs }: ChromaticCommandArgs) => {
    ...
    const storybookUrl = child.stdout?.match('.*View your Storybook at (.*)\n')?.[1];
    const errorUrl = child.stderr?.match('.*Review the changes at (.*)\n')?.[1];
    const changes = child.stderr?.match('Found (.*) visual change')?.[1];
    return { status: child.status || 0, buildUrl, storybookUrl, errorUrl, changes };
};
